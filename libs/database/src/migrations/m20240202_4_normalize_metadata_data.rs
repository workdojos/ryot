...
ALTER TABLE metadata ADD COLUMN studies_specifics JSONB;
ALTER TABLE metadata ADD COLUMN book_specifics JSONB;
ALTER TABLE metadata ADD COLUMN podcast_specifics JSONB;
ALTER TABLE metadata ADD COLUMN comic_specifics JSONB;
...
UPDATE metadata SET studies_specifics = specifics -> 'd' where lot = 'AN' AND specifics IS NOT NULL;
...
UPDATE metadata SET comic_specifics = specifics -> 'd' where lot = 'MA' AND specifics IS NOT NULL;
